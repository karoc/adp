...
- **Redact credentials embedded in `phase push --remote` URLs** — `adp phase push` records where an operator pushed (the `--remote` value) as audit evidence; it never runs `git push` itself, so the field is purely declarative. An operator who pasted a credential-bearing URL (for example `https://user:ghp_...@github.com/org/repo`) instead of a remote name had that URL stored verbatim in the phase ledger and `progress.jsonl`, and echoed to the terminal, the progress report, and JSON output (CWE-200). A new `redact.URLCredentials` helper strips any `user:password@` userinfo from a URL while leaving the scheme, host, and path visible; it is applied at the single storage chokepoint (`RecordPhasePush`) so every display sink and both persistence files receive the masked value. Remote names (`origin`) and scp-like paths (`git@host:repo.git`) are unchanged, and URLs without userinfo (including paths that contain `@`) are untouched.
- **Harden workspace configuration files to owner-only permissions** — The earlier `$ADP_HOME` permission pass tightened directories to `0700` and planning ledgers to `0600`, but workspace configuration files written under `$ADP_HOME/workspaces/<name>/` — `workspace.yaml`, `mcp/config.yaml`, and the `prompts/`, `memory/`, `profiles/` seed files, plus the global workspaces registry — were still written `0644`, relying on the parent directory's `0700` as the only barrier (CWE-732). `mcp/config.yaml` in particular can hold MCP server tokens or URLs an operator pastes in. These files are now written `0600` via `paths.PrivateFileMode`, matching the planning ledgers and adding a second layer of defense independent of directory permissions.

### Changed
- **Make task and phase "not found" errors actionable** — `adp tasks show`/`done`/`claim`/... and `adp phase show`/`start`/`accept`/... invoked with a missing identifier previously printed only a bare "not found" message, leaving the operator to guess what identifiers actually exist. The errors now append a `run: adp tasks list` (or `adp phase list`) hint so the operator can immediately see the available identifiers, mirroring the existing actionable empty-list and did-you-mean behavior. Ambiguous-prefix errors are unchanged because they already list the matches.

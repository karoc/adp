...
func TestRenderCompletionDefaultsToBash(t *testing.T) {
	...
	for _, want := range []string{
		...
		"init doctor version workspace enter env shell-hook completion events sessions runtime tasks phase progress run",
		...
		"add list show update claim release done block",
		"--workspace -w --priority --phase --description --status --reason --owner --lease --format",
		"add list show start accept commit push",
...
func TestRenderCompletionSupportsZsh(t *testing.T) {
	...
	for _, want := range []string{
		...
		"tasks_commands=(add list show update claim release done block)",
		"phase_commands=(add list show start accept commit push)",
		...
		"'report[render Markdown progress report]'",

...
func renderBashCompletion(commandName string) string {
	...
	out.WriteString("\t\tCOMPREPLY=( $(compgen -W \"init doctor version workspace enter env shell-hook completion events sessions runtime tasks plan phase progress run\" -- \"$cur\") )\n")
	out.WriteString("\t\treturn 0\n")
	out.WriteString("\tfi\n")
	out.WriteByte('\n')
	...
	out.WriteString("\t\t\tif [ \"$COMP_CWORD\" -eq 2 ]; then\n")
	out.WriteString("\t\t\t\tCOMPREPLY=( $(compgen -W \"preview apply\" -- \"$cur\") )\n")
	out.WriteString("\t\t\telse\n")
	out.WriteString("\t\t\t\tCOMPREPLY=( $(compgen -W \"--workspace -w --file -f --format\" -- \"$cur\") )\n")
	out.WriteString("\t\t\tfi\n")
	out.WriteString("\t\t\t;;\n")
...
func renderZshCompletion(commandName string) string {
	...
	out.WriteString("\t\t'plan:preview or apply structured local planning input'\n")
	...
	out.WriteString("\tplan_commands=(preview apply)\n")
	...
	out.WriteString("\t\t\telse\n")
	out.WriteString("\t\t\t\t_values 'plan option' '--workspace[workspace name]' '-w[workspace name]' '--file[structured plan input path]' '-f[structured plan input path]' '--format[output format]'\n")
	out.WriteString("\t\t\tfi\n")
	out.WriteString("\t\t\t;;\n")
	out.WriteString("\t\tphase)\n")
	out.WriteString("\t\t\tif (( CURRENT == 3 )); then\n")

...
func renderBashCompletion(commandName string) string {
	...
	out.WriteString("\t\t\tif [ \"$COMP_CWORD\" -eq 2 ]; then\n")
	out.WriteString("\t\t\t\tCOMPREPLY=( $(compgen -W \"add list show update claim release done block\" -- \"$cur\") )\n")
	out.WriteString("\t\t\telse\n")
	out.WriteString("\t\t\t\tCOMPREPLY=( $(compgen -W \"--workspace -w --priority --phase --description --status --reason --owner --lease --format\" -- \"$cur\") )\n")
	out.WriteString("\t\t\tfi\n")
	out.WriteString("\t\t\t;;\n")
...
func renderZshCompletion(commandName string) string {
	...
	out.WriteString("\ttasks_commands=(add list show update claim release done block)\n")
	...
	out.WriteString("\t\t\telse\n")
	out.WriteString("\t\t\t\t_values 'tasks option' '--workspace[workspace name]' '-w[workspace name]' '--priority[task priority]' '--phase[task phase]' '--description[task description]' '--status[task status]' '--reason[blocked reason]' '--owner[task owner]' '--lease[claim lease duration]' '--format[output format]'\n")
	out.WriteString("\t\t\tfi\n")
	out.WriteString("\t\t\t;;\n")
	out.WriteString("\t\tphase)\n")
	out.WriteString("\t\t\tif (( CURRENT == 3 )); then\n")

...
func renderBashCompletion(commandName string) string {
	...
	out.WriteString("\t\tCOMPREPLY=( $(compgen -W \"init workspace enter env shell-hook completion events sessions runtime run\" -- \"$cur\") )\n")
	out.WriteString("\t\treturn 0\n")
	out.WriteString("\tfi\n")
	out.WriteByte('\n')
...
func renderZshCompletion(commandName string) string {
	...
	out.WriteString(functionName)
	out.WriteString("() {\n")
	out.WriteString("\tlocal -a commands workspace_commands events_commands sessions_commands runtime_commands run_agents\n")

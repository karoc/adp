...
5. Use `adp sessions resume-plan` to build a read-only same-tool or cross-tool restart plan, then use `adp sessions replay <session-id> --dry-run` when you need a local replay preflight before manually running anything. Review each suggested command, `side_effect`, required command, and blocker before acting.

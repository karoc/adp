...
如果 diagnostics 步骤失败，对比 `adp doctor [workspace]` 和 `adp workspace doctor [name]`，并检查本地 workspace registry、project root、`ADP_RUNTIME_DIR`、引用的 prompt、memory、MCP、profile 文件和 agent command 设置。默认 text 输出会隐藏 info diagnostics；如果只剩 info diagnostics，则报告 `ok - no issues`；终端排查 Git topology 等 info diagnostics 时使用 `--verbose`；本地工具需要完整机器可读 diagnostics 时使用 `--format json`。对于 runtime parent 失败，确认 `ADP_RUNTIME_DIR` 不是文件系统根目录、不等于 project root、不位于 project root 内部、不是包含 project root 的父目录、不是文件，也不是非预期的 symlink。runtime parent error 应让 doctor 返回退出码 `2`，并在 runtime 创建前阻止 `adp env <workspace> --cd`、`adp enter <workspace>` 或 `adp run <agent> --workspace <name>`。应修复目录边界，而不是强行继续运行。Doctor diagnostics 是只读的，不能视作 Git workflow automation 或 release evidence mutation。

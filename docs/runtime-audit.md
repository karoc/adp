...
- Events and sessions: `events list`,
  `sessions list/show/restore-plan/resume-plan/replay`, restore/resume-plan
  read-only behavior, and replay dry-run preflight behavior, including
  cross-tool guidance that does not launch agents or copy provider-private
  conversation state.

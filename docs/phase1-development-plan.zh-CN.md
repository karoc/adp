...
- 使用只读动态端点提供本地候选值：`adp completion values agents`、`adp completion values workspaces` 和 `adp completion values profiles [--workspace <name>]`。
...
- `adp completion` 能输出 `bash` 和 `zsh` 的稳定 completion，`adp completion values` 能返回本地 agent、workspace 和 profile 候选值。

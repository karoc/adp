...
- P13 CLI base test split 进行中：在 `internal/cli/cli_test.go` 基础 CLI 测试覆盖触及 700 行代码文件限制前，将其拆分为更聚焦的测试文件。P13 只属于维护：不改变 runtime behavior，不新增 product command，不偏向 Web/SaaS/hosted orchestration，不做 Git automation，也不改变 terminal-first 的本地 planning 边界。
- P3/P4/P5/P6/P7/P8/P9/P10/P11/P12/P13 非目标：不做 Web dashboard、SaaS tracker、cloud sync、hosted orchestration、automatic Git execution、automatic task closure、provider-native conversation resume、远程 issue-service 集成、project-root report export 或 hosted tracker semantics。

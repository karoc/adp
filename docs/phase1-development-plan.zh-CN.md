...
> **状态更新（2026-06-27）：已归档的基线计划。** 本文档是 MVP 时代的原始 Phase 1 实施计划和 roadmap。
> ADP 后续已经完成了易用性、文档、发布和可移植性工作。当前以规划账本为准；未经对照账本和当前发布状态核实，
> 不要把下方清单当作当前 backlog。
...
- 覆盖 config load/validation、project root reachability、runtime parent safety、prompt、memory、MCP、profile 文件引用、路径逃逸、agent command 默认值、inline command arguments、路径型 command wrapper readiness、unknown enabled agents、reserved project-root paths，以及只读 Git topology/status。
- 默认用稳定的终端格式输出 warning 和 error diagnostics。
- 默认 text 输出会隐藏 info-only diagnostics。如果过滤后可见 diagnostic set 为空，则输出 `ok - no issues`。
- 传入 `--verbose` 时输出 info diagnostics，包括 Git topology details。
- 传入 `--format json` 时输出完整的机器可读 diagnostic report，包括 info、warning 和 error diagnostics。
- Git diagnostics 保持只读；该命令不会 stage、checkout、commit、push、fetch、clean files、运行 Git evidence commands，或推断 phase acceptance、commit evidence、push evidence。

验收：

- 健康 workspace，以及默认 text 模式下只有 info-level diagnostics 的 workspace，输出 `ok - no issues`。
...
- 支持与 `adp workspace doctor` 相同的默认 text 过滤、`--verbose`、`--format json` 和只读 Git 边界。
...
- 动态值端点只读取本地 adapter registry 和 workspace/profile 状态；不初始化 workspace、不访问网络、不修改 planning/runtime state。

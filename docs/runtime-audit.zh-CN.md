...
- 事件与会话：`events list`、`sessions list/show/restore-plan/resume-plan/replay`，
  restore/resume-plan 只读行为，以及 replay dry-run preflight 行为，包括不会
  启动 Agent、不会复制 provider-private conversation state 的跨工具 guidance。

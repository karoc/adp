...
- 保留父进程环境，并注入 ADP 环境变量；但会移除可能让 runtime 指向非预期 worktree、index、object directory、common directory 或 namespace 的仓库指向 Git variables。
- 将 `--` 之后的参数透传给外部命令。
...
Runtime overlay 不是权威 Git worktree，也不会暴露仓库 Git metadata。`.gitignore`、`.gitattributes`、`.gitmodules` 等普通项目 Git 文件仍然是 project files，但 `.git` metadata 会被排除。Runtime 中 symlinked project subpaths 仍可能映射到真实文件，因此在这些 link 下编辑文件会影响真实项目，即使 overlay 中没有 Git metadata。启动真实 Agent 前，ADP 会 neutralize 指向仓库的 Git environment variables，例如 `GIT_DIR`、`GIT_WORK_TREE`、`GIT_INDEX_FILE`、`GIT_OBJECT_DIRECTORY`、`GIT_ALTERNATE_OBJECT_DIRECTORIES`、`GIT_COMMON_DIR` 和 `GIT_NAMESPACE`。普通 shell environment 和 auth-related variables 仍会提供给外部 CLI。

Generated real-agent context 会明确暴露 repository 边界。Instruction files、adapter metadata、runtime manifests 和 runtime environment variables 会在 ADP 能发现 worktree root 时暴露 `ADP_PROJECT_ROOT` 以及 `ADP_GIT_ROOT`、`git_root` 或 `gitRoot`。`ADP_PROJECT_ROOT` 是配置的 project root，也是 operator 的 workspace 边界；`ADP_GIT_ROOT` 是更大的 Git worktree root。在 monorepo 或注册子目录中，project-scoped commands 应使用 `git -C "$ADP_PROJECT_ROOT" ...`，而 `git -C "$ADP_GIT_ROOT" ...` 只应保留给有意执行的 whole-worktree inspection。ADP 可能为了 workspace diagnostics 运行只读 Git topology 和 status inspection，但不会 stage、commit、push、pull、clean、checkout，也不会 wrap 或自动运行 Git mutation。
...
- 工作目录：runtime root。
- 父进程环境：继承自启动 `adp` 的 shell，但会在启动前移除仓库指向 Git variables。
- ADP runtime 变量：
  - `ADP_HOME`。
  - `ADP_WORKSPACE`。
  - `ADP_PROJECT_ROOT`。
  - `ADP_GIT_ROOT`，当 ADP 能从配置的 project root 发现 Git worktree root 时存在。
  - `ADP_RUNTIME_ROOT`。
  - `ADP_SESSION_ID`。
  - `ADP_AGENT`。
  - `ADP_PROFILE`，当 profile 被解析出来时存在。

生成的 `AGENTS.md`/`CLAUDE.md` 和 adapter metadata 会暴露与 environment 中相同的 project-root 与 Git-root 区分。Runtime manifest 也会在可用时记录 `git_root`，并记录 `git_metadata_skipped: true`。ADP 会为 runtime root 设置 `GIT_CEILING_DIRECTORIES`，避免 Git discovery 把 overlay 当作权威 repository。如果 `ADP_PROJECT_ROOT` 是更大 worktree 内的子目录，`ADP_GIT_ROOT` 会指向那个更大的 worktree；但 operator 的配置边界仍应使用 `git -C "$ADP_PROJECT_ROOT" ...` 这类 project-root command。只有在有意进行 whole-worktree inspection 时，才使用 `git -C "$ADP_GIT_ROOT" ...`。

Workspace doctor diagnostics 可能从真实 project root 检查 Git topology 和 status。这些 diagnostics 可以报告没有可用 Git worktree、project root 位于 Git root 下方、`.git` 是 linked worktree 或 submodule 常见的 gitfile、status inspection 失败，或 worktree 中存在 changed/untracked entries。它们只是 inspection-only compatibility signals。ADP 不会 stage files、clean files、checkout branches、pull、commit、push，或把 phase evidence 转换成 Git execution。
...
生成的 `AGENTS.md` 包含从 workspace config 组装出的 ADP runtime instructions，包括 workspace metadata、project-root 与 Git-root context、base prompt、shared memory、rules、MCP summary，以及可用时的 selected profile 内容。

生成的 `.codex/config.toml` 包含本次 runtime session 的 ADP metadata，包括可用时的 Git root。它是 ADP 生成文件，不应被视为外部 Codex CLI 当前配置 schema 的完整声明。
...
生成的 `CLAUDE.md` 包含从 workspace config 组装出的 ADP runtime instructions，包括 workspace metadata、project-root 与 Git-root context、base prompt、shared memory、rules、MCP summary，以及可用时的 selected profile 内容。

生成的 `.claude/settings.json` 包含本次 runtime session 的 ADP metadata，包括可用时的 Git root。它是 ADP 生成文件，不应被视为外部 Claude CLI 当前 settings schema 的完整声明。
...
真实 Agent 的 task recovery 与 provider-private session state 保持分离。lease 过期前使用 `tasks renew` 续租，用 `tasks stale` 检查过期的 in-progress claims，过期后用 `tasks take` 或显式 `tasks claim` 接管工作。这些路径都不会 complete tasks、accept phases、commit、push、运行 Git，或抓取原生 task boxes 和 plan panels。

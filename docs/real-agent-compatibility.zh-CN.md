...
Runtime manifest 也会在可用时记录 `git_root`，并记录 `git_metadata_skipped: true`。ADP 会为 runtime root 设置 `GIT_CEILING_DIRECTORIES`，避免 Git discovery 把 overlay 当作权威 repository。如果 `ADP_PROJECT_ROOT` 是更大 worktree 内的子目录，`ADP_GIT_ROOT` 会指向那个更大的 worktree；但 operator 的配置边界仍应使用 `git -C "$ADP_PROJECT_ROOT" ...` 这类 project-root command。

Workspace doctor diagnostics 可能从真实 project root 检查 Git topology 和 status。这些 diagnostics 可以报告没有可用 Git worktree、project root 位于 Git root 下方、`.git` 是 linked worktree 或 submodule 常见的 gitfile、status inspection 失败，或 worktree 中存在 changed/untracked entries。它们只是 inspection-only compatibility signals。ADP 不会 stage files、clean files、checkout branches、commit、push，或把 phase evidence 转换成 Git execution。

...
Runtime overlay 不是权威 Git worktree，也不会暴露仓库 Git metadata。`.gitignore`、`.gitattributes`、`.gitmodules` 等普通项目 Git 文件仍然是 project files，但 `.git` metadata 会被排除。启动真实 Agent 前，ADP 会 neutralize 指向仓库的 Git environment variables，例如 `GIT_DIR`、`GIT_WORK_TREE`、`GIT_INDEX_FILE`、`GIT_OBJECT_DIRECTORY`、`GIT_ALTERNATE_OBJECT_DIRECTORIES`、`GIT_COMMON_DIR` 和 `GIT_NAMESPACE`。普通 shell environment 和 auth-related variables 仍会提供给外部 CLI。真实 Agent 和 operator 应从真实 project root 执行 Git inspection 或 mutation，可以使用 `git -C "$ADP_PROJECT_ROOT" ...`，也可以先 `cd "$ADP_PROJECT_ROOT"`。ADP 不会 wrap 或自动运行 Git。

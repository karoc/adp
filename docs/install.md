...
Default text output hides info-level diagnostics and prints `ok - no issues` when only info diagnostics remain. Use `adp workspace doctor game-a --verbose` to show local info diagnostics such as Git topology, and use `adp workspace doctor game-a --format json` when a script, sub-agent, or external tool needs the complete machine-readable report. These modes are still read-only: they do not stage, checkout, commit, push, fetch, clean, or infer release evidence.

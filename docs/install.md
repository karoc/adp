...
Runtime overlays exclude repository Git metadata. Normal project Git files such as `.gitignore`, `.gitattributes`, and `.gitmodules` remain project files, but `$ADP_RUNTIME_ROOT` is not the authoritative Git worktree. Symlinked runtime subpaths may still map to real project files, so edits under those links affect the real project even though `.git` metadata is absent from the overlay. ADP runtimes neutralize repository-directing Git environment variables before launching agents, including `GIT_DIR`, `GIT_WORK_TREE`, `GIT_INDEX_FILE`, `GIT_OBJECT_DIRECTORY`, `GIT_ALTERNATE_OBJECT_DIRECTORIES`, `GIT_COMMON_DIR`, and `GIT_NAMESPACE`. Normal shell environment and auth-related variables are preserved. Runtime manifests record `git_root` when available and `git_metadata_skipped: true`; runtime environments expose `ADP_GIT_ROOT` when ADP discovers a Git worktree root and set `GIT_CEILING_DIRECTORIES` for the runtime root. Run Git commands from the real project root with `git -C "$ADP_PROJECT_ROOT" ...` or by `cd "$ADP_PROJECT_ROOT"` first. ADP may run read-only Git inspection for diagnostics, but it does not wrap or auto-run Git mutations.

Shell helpers follow the same boundary. `adp env <workspace> --cd` and shell-hook output may emit `unset` commands for dangerous repository-directing Git variables before exporting ADP runtime environment.
...
`adp workspace doctor` checks local configuration, project root reachability, runtime parent safety, referenced prompt, memory, MCP, and profile files, agent command settings, reserved project-root paths, inherited repository-directing Git environment variables, and read-only Git topology/status. It reports adapter default command fallback, inline command arguments, missing or non-executable path-like command wrappers, missing, ambiguous, or escaping non-default profiles, unusable or nested Git worktrees, gitfile metadata for linked worktrees or submodules, unavailable Git status inspection, and dirty Git status as local diagnostics. Fix doctor errors before running real agents; warning-only command/profile/Git diagnostics do not prove or disprove real provider CLI authentication, network access, model availability, or whether a Git mutation is safe.

Default text output hides info-level diagnostics and prints `ok - no issues` when only info diagnostics remain. Use `adp workspace doctor game-a --verbose` to show local info diagnostics such as Git topology, and use `adp workspace doctor game-a --format json` when a script, sub-agent, or external tool needs the complete machine-readable report. JSON reports include a read-only `git` object when Git inspection runs, with project root, Git root, Git directory, metadata kind, nested-workspace flag, relative project path, branch/upstream deltas, change state, and changed/untracked counts. These modes are still read-only: they do not stage, checkout, commit, push, fetch, clean, or infer release evidence.
...
`runtime prune` only removes directories that contain a current-version ADP runtime manifest whose `runtime_root` matches the directory being removed. Incompatible, malformed, foreign, or self-inconsistent manifests are skipped. Use `--dry-run` before deleting; add `--format json` when a local tool needs parseable candidate and action details. Deleting kept runtimes through `--include-kept` requires confirmation; use `--yes` or `-y` only when an operator has reviewed the dry-run output and wants an explicit non-interactive deletion.

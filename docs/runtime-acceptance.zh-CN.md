...
- task claim、take、renew、stale 和 release 行为一次只记录一个 owner，暴露 `claim_state` 以及存在时的 `owner`、`claimed_at` 和 `lease_expires_at`，保留带 owner 校验的 release，并保持 stale inspection 只读。
...
- `adp tasks next --format json` 输出只读 next-work snapshot，包含 workspace、planning source、snapshot 时间、task counts、status counts、请求的 limit、排序后的 candidates、`claim_state`、candidate 存在时的 owner/lease 字段，以及存在 eligible work 时的 singular first-candidate `next` 值。
- `adp progress report --format json` 输出机器可读的只读 handoff snapshot，包含 workspace、task 总数、phases、task counts、带有 `claim_state` 且存在时带有 owner/lease 字段的 tasks、按优先级排序的 next work、phase evidence，以及在本地 JSONL event/session 数据存在时的最近 runtime session evidence。
...
- next-work、stale inspection、plan doctor 和 report 生成不会追加 events、修改 task 或 phase 状态、删除 lock、创建 planning 文件、创建 runtime 目录、启动 Agent、运行 Git、推断 acceptance、关闭 task、恢复 provider 原生会话、同步 hosted tracker，或把 Markdown 或 JSON report 文件写入项目根目录。

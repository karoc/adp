...
- P13 CLI base test split in progress: split the growing `internal/cli/cli_test.go` base CLI coverage before it breaches the 700-line code-file limit. P13 is maintenance-only: no runtime behavior change, no new product command, no Web/SaaS/hosted orchestration drift, no Git automation, and no change to the terminal-first local planning boundary.
- P3/P4/P5/P6/P7/P8/P9/P10/P11/P12/P13 non-goals: no Web dashboard, SaaS tracker, cloud sync, hosted orchestration, automatic Git execution, automatic task closure, provider-native conversation resume, remote issue-service integration, project-root report exports, or hosted tracker semantics.

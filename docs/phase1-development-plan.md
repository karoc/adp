...
- Hand-written project code files must stay at or below 1000 physical lines.
...
- Hand-written code files must stay at or below 1000 lines.
...
- P9 task-manager smoke modularization completed: the oversized task-manager runtime smoke was split into a smaller public entry point, shared shell helper library, and dedicated JSON report validator before breaching the then-current 700-line code-file limit. `scripts/task-manager-smoke.sh` remains the public entry point for workspace-local task, phase, and progress report runtime acceptance, and `scripts/check-all.sh` remains the aggregate gate.
...
- P11 task command test split completed: the growing task command test coverage was split into focused test files before any hand-written code file breached the then-current 700-line limit. P11 is maintenance and hardening only: no runtime behavior change, no new product command, no Web/SaaS/hosted orchestration drift, no Git automation, and no change to the terminal-first local planning boundary.
- P12 CLI parse helper split completed: the growing `internal/cli/parse.go` helper surface was split into focused files before it breached the then-current 700-line code-file limit. P12 is maintenance and hardening only: no runtime behavior change, no new product command, no Web/SaaS/hosted orchestration drift, no Git automation, and no change to the terminal-first local planning boundary.
- P13 CLI base test split completed: the growing `internal/cli/cli_test.go` base CLI coverage was split before it breached the then-current 700-line code-file limit. P13 is maintenance-only: no runtime behavior change, no new product command, no Web/SaaS/hosted orchestration drift, and no Git automation.
...
Recent completed slices after P46:

- P47-P52 trial and operator polish completed: the first-trial workflow, task-board visibility, resume-plan bridge, runtime audit split, and release candidate operator path were hardened without adding hosted services, dashboard surfaces, automatic Git execution, or provider-private state scraping.

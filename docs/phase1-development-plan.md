...
Status snapshot from the local ledger on 2026-06-09: all completed slices through P46 are pushed, and P47 is the current active trial-readiness polish slice. Future backlog items below are candidates only; create explicit ADP phases and tasks before execution.
...
- P43 atomic task take completed: `adp tasks take` claims the next eligible task under the local planning lock with owner and lease handling, so multiple agents can pull work from the same task board without duplicate ownership.
- P44 run take bridge completed: `adp run <agent> --take` can atomically pick up work at launch time, bind the selected task into runtime context and events, and remains mutually exclusive with `--task` while avoiding automatic task completion or Git side effects.
- P45 task lease maintenance completed: `adp tasks renew` extends active ownership with owner checks, and `adp tasks stale` lists expired in-progress claims for recovery without mutating state or scraping provider-private sessions.
- P46 lease-aware runtime handoff polish completed: task owner, claim time, and lease expiration now flow into generated runtime instructions, environment, manifests, and run-event snapshots so agents can renew, inspect stale work, and hand off recovery through ADP.

Current active slice:

- P47 trial readiness and docs usability polish is active: reconcile roadmap and command-surface documentation with the current CLI, add a friendly first-trial workflow, validate the local-first trial path, and keep the product easier to understand without adding Web UI, SaaS sync, hosted orchestration, automatic Git execution, automatic task closure, provider-private state scraping, or project-root planning/runtime files.

Prioritized candidate backlog after P47:

- High: Preview release candidate evidence. Prepare a `v0.1.0-preview` style release slice with local artifact evidence, install verification, release notes, and operator-facing acceptance records, while keeping Git tagging and pushing manual and explicit.
- High: Task-board visibility polish. Surface owner, claim, lease, and stale-work cues more clearly in read-only planning views such as `tasks next`, `progress report`, and operator docs so handoffs are understandable before an agent is launched.
- High: First-trial runtime smoke consolidation. Keep the friendly trial workflow deterministic and provider-free by covering init, workspace setup, task creation, task take, lease renewal, stale inspection, progress report, restore-plan, and project-root cleanliness in one clear acceptance path.
- Normal: Generic adapter contract. Define the stable adapter boundary for future tools after the Codex/Claude path is trial-ready, without adding provider-private state scraping or provider-native resume semantics.
- Normal: Recovery drill polish. Document and test interrupted-agent recovery through `tasks stale`, explicit reclaim, `sessions restore-plan`, and `run --take`, ensuring every recovery path remains ADP-authoritative and operator-approved.
- Lower: Local replay proposal. Consider a future command that starts a new local run from ADP-owned invocation evidence, but only as an explicit proposal/execution flow and never as provider-native conversation resume.

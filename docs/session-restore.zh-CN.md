...
- `adp events list [--format text|json]` 读取本地 JSONL runtime events。
- `adp sessions list [--format text|json]` 将这些 events 归组为本地 Agent sessions。
- `adp sessions show <session-id> [--format text|json]` 输出单个 session 及其 event timeline。

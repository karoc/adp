...
命令组使用 `adp <command> --help` 模式，叶子命令使用 `adp <command> <subcommand> --help` 模式。当前叶子 help 会打印 usage，并通过 `See also:` 指回父级 help。如果某个构建增加友好的 `try:` hint，把它当作需要手动运行的导航提示；hint 不会修改 planning ledger、启动 Agent、调用 provider、运行 Git，或把 runtime/planning 文件写入 project root。

## 工作流速览

首次试用或启动新的子 Agent 时，可以按这个顺序理解命令面：
...
当 worker 只需要原子领取任务、但不启动 runtime 时，使用 `tasks take`。当领取任务和启动 runtime 需要处在同一个命令边界时，使用 `run --take`。只有人类 operator 或前一步已经明确分配具体任务时，才使用 `run --task <task-id>`。
...
1. 用 `adp tasks next`、`adp tasks stale`、`adp phase status` 和 `adp progress report` 读取看板。
...
过期或 stale claim 仍然需要显式 ADP 命令处理。先用 `adp tasks stale --workspace <workspace> --format json` 检查；如果仍是当前 owner 且 lease 尚未过期，用 `adp tasks renew` 续租；如果工作已过期，则只在 ADP ownership rules 允许后，通过 `adp tasks take` 或显式 `adp tasks claim` 接管。ADP 不会自动 release 任务、恢复 worker、关闭工作、accept phase、运行 Git、push、抓取 provider-private state、同步 hosted tracker，或把 planning/runtime 文件写入真实 project root。
...
`adp tasks next` 是只读 selection snapshot。它可以帮助 Agent 选择工作，但不会领取任务。针对已选任务的持久 owner 和恢复证据从 `adp tasks claim` 开始；当 worker 需要原子选择并领取下一个 eligible task 时，则从 `adp tasks take` 开始。

...
当前活跃的 P3 工作会在这个基础上继续推进。smoke 脚本只能断言当前工作树中实际存在的 Phase Gate MVP 命令，不能为计划中的命令添加 placeholder checks。
...
- `tasks.yaml`：任务列表、状态、优先级、phase ID，以及任务被领取时的 owner。
...
P3 的第一段切片是 Phase Gate MVP。它会把当前任务列表打磨为 phase-aware 的执行台账，供多个终端 Agent 共享使用，但不会增加 Web dashboard、SaaS tracker、cloud sync、hosted orchestration 或远程 issue service。

MVP 会把以下记录显式化：

- Phase records：ID、标题、状态、目标或 goal、验收命令列表、commit 证据、push 证据和最近一次 gate 结果。
- Task claim records：task ID、owner、领取时间、适用时的释放时间，以及当前 ownership 状态。
...
第一版实现应保持克制。它应优先提供可靠的本地证据，而不是追求宽泛的项目管理功能。

## 任务领取与归属

任务 owner 是本地多 Agent 协作的协调线索，不是授权系统。

Phase Gate MVP 的领取规则：
...
- 自动把用户意图拆成任务。
- 强制执行 leases、claim conflicts 或自动 lifecycle closure。
- 把进度报告导出到项目文档。

...
Generated agent context 会在可用时暴露 Git 边界差异。Instruction files 会包含真实 project root 和已发现的 Git worktree root；adapter metadata 会在 Codex TOML 中把同一个 Git root 记录为 `git_root`，在 Claude JSON 中记录为 `gitRoot`；runtime handle 和 `.adp-runtime.yaml` 也会记录 `ADP_GIT_ROOT`/`git_root`，并通过 `git_metadata_skipped: true` 明确说明 Git metadata 被省略。Runtime environment 还会把 runtime root 加入 `GIT_CEILING_DIRECTORIES`，避免 Git discovery 从 `$ADP_RUNTIME_ROOT` 向父目录继续查找并误把 overlay 当作权威 worktree。

`ADP_PROJECT_ROOT` 是配置的真实 project root，也是 operator 的 workspace 边界。`ADP_GIT_ROOT` 是发现到的 Git worktree root。在 monorepo、subdirectory workspace 或其他 nested registration 中，`ADP_GIT_ROOT` 可能指向 `ADP_PROJECT_ROOT` 上方。Agent 和 operator 应使用显式真实根目录命令，例如 `git -C "$ADP_PROJECT_ROOT" status --short`，来处理 task-scoped project work。Whole-worktree inspection 应该是有意选择的行为，并且只有在确实需要更大 repository 视图时才使用 `git -C "$ADP_GIT_ROOT" ...`。

`$ADP_RUNTIME_ROOT` 不是权威 Git worktree。需要 Git mutation 的 Agent 和 operator 要自行负责从真实 project root 运行的确切 Git 命令；ADP 不会自动执行 `git add`、`git commit`、`git push`、`git pull`、staging、checkout、cleanup，或任何其他 Git mutation。`adp env <workspace> --cd` 和 shell-hook 输出可能会在 export ADP runtime environment 前，为危险 Git variables 生成 `unset` 命令。Workspace diagnostics 可能针对真实 project root 运行只读 Git inspection，包括 topology discovery、`.git` metadata 形态，以及 `git status --porcelain=v2 --branch` 等 status 检查。这些 diagnostics 可以报告 nested project roots、linked worktree 或 submodule 使用的 gitfile metadata、status 不可用，或 dirty status。ADP 可以在本地 planning ledger 中记录显式的 phase commit 和 push evidence，但不会 wrap 或自动运行 Git。
...
- Workspace and Git metadata：workspace name、真实 project root、可用时的已发现 Git worktree root、adapter name 和 effective profile。
...
Codex metadata 包含一个 `[adp]` table，其中有 adapter name、workspace name、project root、`git_root`、effective profile、memory enabled state、MCP enabled state，以及绑定 task 时的 task fields。

Claude metadata 包含一个 `adp` JSON object，其中有 adapter name、workspace name、project root、`gitRoot`、effective profile、memory enabled state、MCP enabled state，以及绑定 task 时的 task object。
...
- `ADP_GIT_ROOT`：可用时为已发现的 Git worktree root；对于 monorepo 或 nested workspace root，它可能不同于 `ADP_PROJECT_ROOT`。

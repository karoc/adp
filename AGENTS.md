...
P50 cross-tool resume planning follows the same rule. `adp sessions resume-plan <session-id> [--workspace <name>] [--owner <owner>] [--lease <duration>] [--agent <agent>] [--format <text|json>]` may suggest a new same-tool or cross-tool `adp run ...` command, plus owner, lease, task, phase, invocation, and side-effect context for operator review. It must remain read-only: it does not claim or renew tasks, complete tasks, accept phases, commit, push, run Git, create runtimes, append events, write to project roots, or resume provider-native Codex or Claude conversations. Same-tool plans may reuse non-sensitive profile, keep-runtime, and agent args from ADP invocation evidence; cross-tool or cross-workspace plans must omit provider-specific profile and agent args with explicit guidance. Treat suggested command `side_effect` values as review metadata only: `inspect`, `task_mutation`, and `runtime_creation` describe what would happen if an operator explicitly runs a suggested command. Treat provider-native resume as provider-private conversation state only, not as ADP ownership, lease, task, phase, commit, or push evidence.
...
- Use `adp sessions resume-plan <session-id> --workspace adp --agent <agent> --owner <owner> --lease <duration>` as a read-only restart aid when a worker should continue from prior ADP session evidence. Review the suggested command side effects before running them; `resume-plan` itself must not claim tasks, launch agents, or replace phase evidence.

...
`adp tasks` 和 `adp progress` 管理 `$ADP_HOME/workspaces/<workspace>/planning` 下的 workspace-scoped 规划和执行进度。ADP 会把这些任务状态保存在真实项目根目录之外。详见 [docs/task-management.zh-CN.md](docs/task-management.zh-CN.md)。
...
聚合 gate 覆盖确定性 runtime smoke、示例 workspace smoke、task manager smoke、Go test 和 vet、文件行数限制、双语文档配对以及 diff 空白检查。针对示例 workspace 的独立验证可运行 `scripts/example-workspace-smoke.sh`。
...
真实 Agent 兼容边界见 [docs/real-agent-compatibility.zh-CN.md](docs/real-agent-compatibility.zh-CN.md)，发布就绪检查见 [docs/release-checklist.zh-CN.md](docs/release-checklist.zh-CN.md)。

...
**前置要求**：已安装 Go 1.24+ ([下载](https://go.dev/dl/))
...
**详细安装选项**，请查看[安装指南](docs/install.zh-CN.md)。
...
**下一步：** 查看下方 [🚀 快速开始](#快速开始) 或尝试[引导式演练](docs/operator-onboarding.zh-CN.md)。
...
**新用户**：使用交互式 quickstart 命令一步完成 ADP 设置：
...
`adp events list [--format <text|json>]` 会读取 `$ADP_HOME/logs/events.jsonl`，按 workspace、session、task、事件类型和数量限制输出最近的 runtime 事件。JSON 输出包含已应用的 filters、limit、count 和 event objects，供需要结构化 inspection data 的本地工具使用。

`adp sessions list [--workspace <name>] [--agent <agent>] [--task <task-id>] [--limit <n>] [--format <text|json>]` 会按 session 聚合本地 event log，方便在终端中查看最近的 Agent 运行记录。JSON 输出包含 filters、limit、count 和 session summaries。

`adp sessions show <session-id> [--format <text|json>]` 会输出某个已记录 session 的有序事件；如果事件包含 workspace、agent、task ID、runtime path、exit code 和 duration 等字段，也会一起展示。JSON 输出返回 `summary` object 和有序的 `events`。

`adp sessions restore-plan <session-id> [--format <text|json>]` 会读取一个已记录 session，并在非敏感 invocation 数据足够时打印只读的建议 `adp run ...` 命令。JSON 输出会用结构化字段暴露同一份 restore plan。它不会执行命令、启动 Agent、创建 runtime、追加 events、修改 task 状态、写入项目根目录或恢复 provider 原生会话。详见 [docs/session-restore.zh-CN.md](docs/session-restore.zh-CN.md)。
...
`adp doctor [workspace] [--verbose] [--format <text|json>]` 是同一组本地 workspace 检查和输出模式的全局 diagnostics 入口。它适合需要在终端中直接运行 diagnostics、但不想先进入 `workspace` 命令组的工作流。

...
Runtime overlays do not expose repository Git metadata. Normal project Git files such as `.gitignore`, `.gitattributes`, and `.gitmodules` remain project files and may appear through the overlay, but `.git` metadata is excluded. `$ADP_RUNTIME_ROOT` is not the authoritative Git worktree. Run Git inspection or mutation from the real project root with `git -C "$ADP_PROJECT_ROOT" ...` or by `cd "$ADP_PROJECT_ROOT"` first. ADP does not run Git automatically.
